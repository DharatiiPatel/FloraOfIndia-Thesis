...
> - The clean-set **elevation** values and the clean **χ²=21.96** have **no surviving
>   source** in the tree and cannot currently be verified.
>
> The inputs still exist (`step05b_outputs_clean/`, n=1,174 with elevation on all
> rows), so these can be regenerated by re-running steps 06 and 08 against the clean
> input. Until then, **cite the n=1,438 column; treat the n=1,174 column as
> historical.**
