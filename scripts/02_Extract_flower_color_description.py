...
OUTPUT_CSV = BASE_DIR / "Processed Data" / "flora_of_india_flower_color_llama3.csv"
...
def load_model_and_tokenizer():
    ...
    if not hf_token:
        raise RuntimeError(
            "HF_TOKEN environment variable is not set. Export your Hugging Face token in the SLURM script."
        )
...
def infer_flower_color(tokenizer, model, description: str) -> str:
    """Call Llama-3 with a chat prompt and return its short answer text."""
    messages = [
    ...
    # Only keep generated tokens (after the prompt)
